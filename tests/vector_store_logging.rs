...
fn test_redis_vector_store_logs_upsert() {
    ...
    let _ = store.upsert(entity_type, key, &vector);
    let logs = logs.lock().unwrap();
    let found = logs.iter().any(|l| l.contains("upsert") && l.contains("Redis"));
    assert!(found, "Redis upsert should log an info message");
}
